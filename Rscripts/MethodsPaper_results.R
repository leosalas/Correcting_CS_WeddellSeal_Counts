...
getGeneralSummary<-function(){
	...
	crd<-merge(crd,views[,c("mapViewId","mapId")],by="mapViewId",all.x=T)
	crd<-merge(crd,maps[,c("mapId","overlayId")],by="mapId",all.x=T)
	ww<-subset(overlays, overlayId %in% unique(crd$overlayId))
	ww$acDate<-format(ww$acquisition_date,"%Y-%m-%d")
	aa<-unique(ww$acDate)
	print("How many acquired satellite images?")
	print(NROW(subset(aa,grepl("2010",aa)| grepl("2011",aa))))
...
load(file="//prbo.org/Data/Home/Petaluma/lsalas/Documents/lsalas/Antarctica/SealsFromSpace/Data/Erebus/dataFromGeoJson3.RData")
mlMaps<-data.frame(mapId=unique(subset(views,taggerId==mid)$mapId))

#Get the number of tags per map for maps inspected by Michelle
mlTags<-subset(tags,taggerId==mid,select=c("taggerId","mapViewId"))
mlCounts<-aggregate(taggerId~mapViewId,mlTags,NROW);names(mlCounts)<-c("mapViewId","mlCount")
mlCounts<-merge(mlCounts,views[,c("mapViewId","mapId")],by="mapViewId",all.x=T)
mlMapCounts<-aggregate(mlCount~mapId,mlCounts,sum,na.rm=T)
#Add the 0's
mlCountdf<-merge(mlMaps,mlMapCounts,by="mapId",all.x=T)
...
crData<-merge(crowd,tags[,c("tagId","mapViewId")],all.x=T)
crData<-merge(crData,views[,c("mapViewId","mapId")],by="mapViewId",all.x=T)
...
for(crf in crfvals){
	i<-i+1
	crft<-as.character(crf);crft<-ifelse(nchar(crft)==4,crft,paste0(crft,"0"))
	tdf<-subset(crData,score >= crf)
	ccdf<-aggregate(sensor~mapId,tdf,NROW);names(ccdf)<-c("mapId","crCount")
	ccdf$threshold<-paste0("Threshold=",crft)
	ccdf<-merge(ccdf,mlCountdf,by="mapId",all.x=T)
	ccdf<-subset(ccdf,!is.na(mlCount))
	dat<-rbind(dat,ccdf)
	mdl<-lm(crCount~mlCount,ccdf)
	preds<-predict(mdl,se.fit=T)
	mdls[[i]]<-list(mdl=mdl,threshold=crf)
	smdl<-summary(mdl)
	mdlrt<-data.frame(threshold=crf,adjRsq=smdl$adj.r.squared,slope=smdl$coefficients[2,1],intercept=smdl$coefficients[1,1],numMaps=nrow(ccdf),mlCount=sum(ccdf$mlCount),crCount=sum(ccdf$crCount),seCrCount=sum(preds$se.fit))
...
for(crt in crtvals){
	i<-i+1
	crtt<-as.character(crt);crtt<-ifelse(nchar(crtt)==4,crtt,paste0(crtt,"0"))
	tdf<-subset(pdfs_ml,featuresShared==20 & round(crankThreshold,2)==round(crt,2))
	mdl<-lm(estSeals~countML,tdf)
	preds<-predict(mdl,se.fit=T)
	mdlsh[[i]]<-list(mdl=mdl,threshold=crt)
	smdl<-summary(mdl)
	mdlrt<-data.frame(threshold=crtt,adjRsq=smdl$adj.r.squared,slope=smdl$coefficients[2,1],intercept=smdl$coefficients[1,1],
			numMaps=nrow(tdf),mlCount=sum(tdf$countML),crEstimate=sum(tdf$estSeals),seCrCount=sum(preds$se.fit),meanShrinkage=mean(tdf$meanShrinkage),numMapViews=sum(tdf$numTaggers))
...
### Printing out
write.csv(mdlres,file="//prbo.org/Data/Home/Petaluma/lsalas/Documents/lsalas/Antarctica/SealsFromSpace/MethodsPaper/FinalFigures/Table1_final.csv")
write.csv(mdlresh,file="//prbo.org/Data/Home/Petaluma/lsalas/Documents/lsalas/Antarctica/SealsFromSpace/MethodsPaper/FinalFigures/Table2_final.csv")
jpeg(filename = "//prbo.org/Data/Home/Petaluma/lsalas/Documents/lsalas/Antarctica/SealsFromSpace/MethodsPaper/FinalFigures/Figure2_final.jpg",width=2200,height=1500,res=300,quality=100)
	print(p1)
dev.off()
jpeg(filename = "//prbo.org/Data/Home/Petaluma/lsalas/Documents/lsalas/Antarctica/SealsFromSpace/MethodsPaper/FinalFigures/Figure3_final.jpg",width=2200,height=1500,res=300,quality=100)
	print(p2)
dev.off()
